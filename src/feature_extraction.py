...
def aggregate_row(row):
    ...
    if non_zero_values.empty:
        ...
        for q in deciles:
            aggregations['non_zero_percentile_{}'.format(q)] = np.nan
            aggregations['non_zero_median_diff_percentile_{}'.format(q)] = np.nan
            aggregations['non_zero_log_percentile_{}'.format(q)] = np.nan
            aggregations['non_zero_log_median_diff_percentile_{}'.format(q)] = np.nan
        ...
        for q in deciles:
            ...
            aggregations['non_zero_median_diff_percentile_{}'.format(q)] \
                = aggregations['non_zero_median'] - aggregations['non_zero_percentile_{}'.format(q)]

            aggregations['non_zero_log_percentile_{}'.format(q)] = np.percentile(np.log1p(non_zero_values), q=q)
            aggregations['non_zero_log_median_diff_percentile_{}'.format(q)] \
                = aggregations['non_zero_log_median'] - aggregations['non_zero_log_percentile_{}'.format(q)]
