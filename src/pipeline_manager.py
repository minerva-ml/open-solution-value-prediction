...
from .utils import init_logger, NeptuneContext, set_seed, \
    ...
    root_mean_squared_log_error, KFoldByTargetValue, read_oof_predictions
...
def train_evaluate_cv_second_level(pipeline_name):
    ...
    train, test = read_oof_predictions(params.first_level_oof_predictions_dir,
                                       ...
                                       id_column=cfg.ID_COLUMN[0],
                                       target_column=cfg.TARGET_COLUMN[0])
    fold_scores = []
...
def _fold_fit_evaluate_predict_loop(train_data_split, valid_data_split, test, fold_id, pipeline_name, model_level):
    score, y_valid_pred, pipeline = _fold_fit_evaluate_loop(train_data_split, valid_data_split,
                                                            fold_id, pipeline_name, model_level)

    if model_level == 'first':
        drop_columns = cfg.ID_COLUMN
    elif model_level == 'second':
        drop_columns = cfg.ID_COLUMN + ['fold_id']
    else:
        raise NotImplementedError

    test_data = {'input': {'X': test.drop(drop_columns, axis=1),
                           'y': None,
                           },
                 }
...
def _fold_fit_evaluate_loop(train_data_split, valid_data_split, fold_id, pipeline_name, model_level):
    if model_level == 'first':
        drop_columns = cfg.TARGET_COLUMN + cfg.ID_COLUMN
    elif model_level == 'second':
        drop_columns = cfg.TARGET_COLUMN + cfg.ID_COLUMN + ['fold_id']
    else:
        raise NotImplementedError
